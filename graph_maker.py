...
def read_test_times(test_conditions_file: Path) -> tuple[pd.Timestamp, pd.Timestamp, dict[str, str]]:
    ...
    if "timestamp" not in df.columns or "status" not in df.columns:
        raise ValueError("testcondities.csv moet de kolommen timestamp en status bevatten.")

    df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce")
    df["status_clean"] = df["status"].astype(str).str.strip().str.lower()
    df = df.dropna(subset=["timestamp"]).sort_values("timestamp")

    started_rows = df[df["status_clean"] == "gestart"]
    if started_rows.empty:
        raise ValueError("Geen status 'gestart' gevonden in testcondities.csv.")

    start_row = started_rows.iloc[0]
    sensor_start = start_row["timestamp"]

    stopped_rows = df[(df["status_clean"] == "gestopt") & (df["timestamp"] >= sensor_start)]
    if stopped_rows.empty:
        raise ValueError("Geen status 'gestopt' gevonden na 'gestart' in testcondities.csv.")

    sensor_end = stopped_rows.iloc[0]["timestamp"]

    labels = DEFAULT_SENSOR_LABELS.copy()
    for i in range(1, 5):
        col = f"locsensor{i}"
        if col in df.columns:
            value = str(start_row.get(col, "")).strip()
